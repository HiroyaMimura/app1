  ...
  describe"投稿画面(new_list_path)のテスト"do
    ...
    context'投稿処理のテスト'do
      it'投稿後のリダイレクト先は正しいか'do
        fill_in'list[title]',with:Faker::Lorem.characters(nunber:5)
        fill_in'list[body]',with:Faker::Lorem.characters(nunber:20)
        click_button'投稿'
...
describe"詳細画面のテスト"do
  ...
  context'list削除のテスト'do
    it'listの削除'do
  context{list.destroy}.to change{List.count}.by(-1)
  end
 end
end
